...
export function Badge({ variant = "default", children, className }: BadgeProps) {
  return (
    <span className={cn("status-badge font-medium", VARIANTS[variant], className)}>{children}</span>
  );
}

// Badge de statut avec label automatique (pratique pour les listes)
export function StatusBadge({ status }: { status: string }) {
  return <Badge variant={status as BadgeVariant}>{STATUS_LABELS[status] ?? status}</Badge>;
}
